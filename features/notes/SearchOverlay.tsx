...
import { highlightSegments, type Note } from '@/lib/notes'
...
export default function SearchOverlay({
  ...
  return (
    <div className="fixed inset-0 z-40 bg-[var(--paper)] animate-fade-up" style={{ paddingTop: 'max(0.75rem, env(safe-area-inset-top))' }}>
      <form
      ...
      <div className="px-4 pt-4">
        {recents.length > 0 && (
          <>
            <p className="text-[0.65rem] font-medium uppercase tracking-[0.16em] text-[var(--muted)]">Recent</p>
            ...
            {hits.slice(0, 8).map((note) => (
              ...
              >
                {highlightSegments(note.title || 'Untitled', value).map((part, index) => (
                  <span key={index} className={part.match ? 'font-semibold' : undefined}>{part.text}</span>
                ))}
              </button>
            ))}
            {hits.length === 0 && (
